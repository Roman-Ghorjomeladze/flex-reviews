...
export const reviewsApi = {
	...
	getPropertyStats: async (filters?: PropertyFilters): Promise<PaginatedResponse<PropertyStats>> => {
		...
		const response = await api.get<{ status: string; properties: PropertyStats[]; traceId?: string }>(url);
	...
	getChannels: async (): Promise<string[]> => {
		const response = await api.get<{ status: string; channels: string[]; traceId?: string }>("/reviews/channels");
	...
	getUser: async (
		...
		try {
			const response = await api.get<{
				status: string;
				user: { userId: string; userName: string; reviewCount: number; averageStars: number | null };
				traceId?: string;
			}>(`/reviews/user/${userId}`);
	 ...
	 */
	getReviewsByUser: async (userId: string, page?: number, limit?: number): Promise<PaginatedResponse<Review>> => {
		const params = new URLSearchParams();
		if (page) params.append("page", String(page));
		if (limit) params.append("limit", String(limit));

		const queryString = params.toString();
		const url = `/reviews/user/${userId}/reviews${queryString ? `?${queryString}` : ""}`;
